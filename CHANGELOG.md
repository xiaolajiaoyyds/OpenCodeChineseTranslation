...
## [1.2.15-zh-v2.8] - 2026-03-01

### 🔧 修复

- **依赖检测逻辑重构**
  - 从硬编码关键包名改为动态读取 `package.json` 的所有 `dependencies` 进行检测
  - 自动识别缺失依赖并提示具体包名，避免误判"依赖已存在"导致编译失败
  - `workspace:` 内部包跳过检查，`catalog:` 和普通版本号均纳入检测

- **安装降级策略**
  - 先尝试 `--frozen-lockfile` 安装，失败后自动降级为普通 `bun install`
  - 解决上游更新后 lockfile 不同步导致安装失败的问题
